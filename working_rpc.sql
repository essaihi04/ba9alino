-- Working RPC: bypass auth.users and use direct insert with defaults
...
BEGIN
    ...
    IF new_user_id IS NULL THEN
        -- Use raw SQL to bypass any column constraints
        EXECUTE format('
            INSERT INTO auth.users (
                ...
                %L, gen_random_uuid(), ''authenticated'', ''authenticated'', 
                %L, crypt(%L, gen_salt(''bf'')),
                NOW(), NOW(), NOW(), NOW(),
                %L, %L
            )',
            v_instance_id, p_email, p_password,
    ...
    ELSE
        ...
        EXECUTE format('
            UPDATE auth.users SET
                encrypted_password = crypt(%L, gen_salt(''bf'')),
                email_confirmed_at = NOW(),
                confirmed_at = COALESCE(confirmed_at, NOW()),
                updated_at = NOW()
            WHERE id = %L
        ', p_password, new_user_id);
    END IF;

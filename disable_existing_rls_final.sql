-- Script INTELLIGENT FINAL pour désactiver RLS seulement sur les tables qui existent
...
BEGIN
    ...
    FOR i IN 1..array_length(table_array, 1) LOOP
        tbl_name := table_array[i];
        
        -- Vérifier si la table existe (avec alias pour éviter l'ambiguïté)
        SELECT EXISTS (
        ...
        IF table_exists THEN
            ...
            AND c.relnamespace = (SELECT oid FROM pg_namespace n WHERE n.nspname = 'public');
            
            IF rls_enabled THEN
                -- Désactiver RLS
                EXECUTE format('ALTER TABLE public.%I DISABLE ROW LEVEL SECURITY', tbl_name);
                tables_with_rls := tables_with_rls + 1;
                RAISE NOTICE '✅ RLS désactivé sur: %', tbl_name;
            ELSE
                RAISE NOTICE 'ℹ️  RLS déjà désactivé sur: %', tbl_name;
            END IF;
        ELSE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'virtual_login') THEN
        GRANT EXECUTE ON FUNCTION public.virtual_login(text, text) TO anon;
        RAISE NOTICE '✅ Permissions activées sur: virtual_login';
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'virtual_create_account') THEN
        -- Vérifier la signature exacte de la fonction
        IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'virtual_create_account' AND p.pronargs = 5) THEN
        ...
        ELSIF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'virtual_create_account' AND p.pronargs = 4) THEN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'virtual_list_accounts') THEN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'virtual_delete_account') THEN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.proname = 'reset_all_sequences') THEN

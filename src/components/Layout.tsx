...
export default function Layout({ children }: LayoutProps) {
  ...
  const menuItems = [
    ...
    { label: 'العائلات', path: '/families', icon: Box, color: 'text-purple-600' },
          ...
          <header className="bg-white shadow-sm px-4 py-2 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {!isDashboard && (
                <button
                  onClick={() => navigate('/')}
                  className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 font-bold text-sm"
                >

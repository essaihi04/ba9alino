...
export default function ProductsPage() {
  ...
  const handlePriceSave = async (moveToNext: boolean = false) => {
    ...
    try {
      ...
      if (moveToNext) {
        ...
        const typedField = field as 'name_ar' | 'stock' | 'price_a' | 'price_b' | 'price_c' | 'price_d' | 'price_e'
        const currentIndex = fields.indexOf(typedField)
  ...
  const handlePriceUpdate = async (productId: string, priceField: 'price_a' | 'price_b' | 'price_c' | 'price_d' | 'price_e', value: string) => {
    ...
    try {
      ...
      setProducts(prev => prev.map(p => 
        p.id === productId ? { ...p, [priceField]: numValue } : p
      ))
      
      console.log(`✅ Updated ${priceField} for product ${productId} to ${numValue}`)
    } catch (error) {
      console.error('Error updating price:', error)
      alert(`❌ خطأ في تحديث السعر: ${error instanceof Error ? error.message : 'Erreur inconnue'}`)
    }
  }

  const handlePriceKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, productId: string, currentField: 'price_a' | 'price_b' | 'price_c' | 'price_d' | 'price_e') => {
    if (e.key === 'Enter') {
      e.preventDefault()
      
      // Define field order
      const fieldOrder: ('price_a' | 'price_b' | 'price_c' | 'price_d' | 'price_e')[] = ['price_a', 'price_b', 'price_c', 'price_d', 'price_e']
      const currentIndex = fieldOrder.indexOf(currentField)
      
      // Find current product index in filtered/paginated list
      const currentProductIndex = paginatedProducts.findIndex(p => p.id === productId)
      
      let nextField: 'price_a' | 'price_b' | 'price_c' | 'price_d' | 'price_e' | null = null
      let nextProductId: string | null = null
      
      if (currentIndex < fieldOrder.length - 1) {
        // Move to next price field in same product
        nextField = fieldOrder[currentIndex + 1]
        nextProductId = productId
      } else if (currentProductIndex < paginatedProducts.length - 1) {
        // Move to first price field of next product
        nextField = fieldOrder[0]
        nextProductId = paginatedProducts[currentProductIndex + 1].id
      }
      
      if (nextField && nextProductId) {
        // Focus next input
        setTimeout(() => {
          const nextInput = document.querySelector(`input[data-product-id="${nextProductId}"][data-field="${nextField}"]`) as HTMLInputElement
          if (nextInput) {
            nextInput.focus()
            nextInput.select()
          }
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" dir="rtl">
          ...
          <div className="h-full flex flex-col">
            ...
            <table className="w-full border border-gray-200 border-collapse">
              <thead className="bg-gray-100 text-black sticky top-0 z-10">
                <tr>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">
                    <input
                      type="checkbox"
                  ...
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">صورة</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">اسم</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">باركود</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">سعر زبون خاص</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">سعر الجملة</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">سعر نصف الجملة</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">سعر مول الحانوت</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">سعر التقسيط</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">مخزون</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">حالة</th>
                  <th className="px-2 py-1 text-right font-bold text-xs border border-gray-200">إجراءات</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2 max-w-[200px]">
                        <div className="font-medium text-gray-900 text-sm truncate" title={product.name_ar}>{product.name_ar}</div>
                      </td>
                      ...
                      <td className="p-2">
                        <input
                          ...
                          step="0.01"
                          min="0"
                          value={product.price_a || ''}
                          onChange={(e) => handlePriceUpdate(product.id, 'price_a', e.target.value)}
                          onKeyDown={(e) => handlePriceKeyDown(e, product.id, 'price_a')}
                          data-product-id={product.id}
                          data-field="price_a"
                          className="w-16 px-1 py-0.5 text-xs border border-gray-300 rounded focus:border-purple-500 focus:outline-none text-right"
                          placeholder="0.00"
                        />
                      </td>
                      <td className="p-2">
                        <input
                          ...
                          step="0.01"
                          min="0"
                          value={product.price_b || ''}
                          onChange={(e) => handlePriceUpdate(product.id, 'price_b', e.target.value)}
                          onKeyDown={(e) => handlePriceKeyDown(e, product.id, 'price_b')}
                          data-product-id={product.id}
                          data-field="price_b"
                          className="w-16 px-1 py-0.5 text-xs border border-gray-300 rounded focus:border-purple-500 focus:outline-none text-right"
                          placeholder="0.00"
                      ...
                      <td className="p-2">
                        <input
                          ...
                          onChange={(e) => handlePriceUpdate(product.id, 'price_c', e.target.value)}
                          onKeyDown={(e) => handlePriceKeyDown(e, product.id, 'price_c')}
                          data-product-id={product.id}
                          data-field="price_c"
                          className="w-16 px-1 py-0.5 text-xs border border-gray-300 rounded focus:border-purple-500 focus:outline-none text-right"
                          placeholder="0.00"
                      ...
                      <td className="p-2">
                        <input
                          ...
                          step="0.01"
                          min="0"
                          value={product.price_d || ''}
                          onChange={(e) => handlePriceUpdate(product.id, 'price_d', e.target.value)}
                          onKeyDown={(e) => handlePriceKeyDown(e, product.id, 'price_d')}
                          data-product-id={product.id}
                          data-field="price_d"
                          className="w-16 px-1 py-0.5 text-xs border border-gray-300 rounded focus:border-purple-500 focus:outline-none text-right"
                          placeholder="0.00"
                      ...
                      <td className="p-2">
                        <input
                          ...
                          step="0.01"
                          min="0"
                          value={product.price_e || ''}
                          onChange={(e) => handlePriceUpdate(product.id, 'price_e', e.target.value)}
                          onKeyDown={(e) => handlePriceKeyDown(e, product.id, 'price_e')}
                          data-product-id={product.id}
                          data-field="price_e"
                          className="w-16 px-1 py-0.5 text-xs border border-gray-300 rounded focus:border-purple-500 focus:outline-none text-right"
                          placeholder="0.00"
                      ...
                      <td className="p-2">
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="bg-purple-100 hover:bg-purple-200 text-purple-700 px-1.5 py-0.5 rounded text-xs font-bold transition-colors"
                          >
                            <Edit2 size={12} />
                          </button>
                          <button
                            onClick={() => handleArchiveOne(product)}
                            className="bg-red-100 hover:bg-red-200 text-red-700 px-1.5 py-0.5 rounded text-xs font-bold transition-colors"
                            disabled={bulkLoading}
      ...
      {showEditProductModal && (
        <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 ${isSavingProduct ? 'pointer-events-none' : ''}`} onClick={() => { if (!isSavingProduct) { setShowEditProductModal(false); setVariants([]); setPrimaryVariants([]); } }}>
          <div className={`bg-white rounded-xl p-6 w-full max-w-6xl max-h-[90vh] overflow-y-auto ${isSavingProduct ? 'opacity-75' : ''} relative`} onClick={(e) => e.stopPropagation()}>
            {isSavingProduct && (
              <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center z-10 rounded-xl">
                <div className="text-center">
                  <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto mb-4"></div>
                  <p className="text-purple-700 font-bold">جاري حفظ التعديلات...</p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleEditProduct} className="space-y-4">
              <div className="sticky top-0 z-20 bg-white py-3 border-b border-gray-200 flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-lg font-bold disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={isSavingProduct}
                >
                  إلغاء
                </button>
